...
st.sidebar.markdown("---")
# Defaulting to Global Insights so dashboard shows data immediately
...
if feature == "Global Insights":
    ...
    with col1:
        ...
        fig = px.line(season_runs, x='season', y='total_runs', title="Total Runs Trend (Season-wise)", markers=True)
...
if feature == "Season Mastery":
    ...
    with c1:
        ...
        st.plotly_chart(px.bar(oc, x='total_runs', y='batter', orientation='h', title="Orange Cap", color='total_runs'), use_container_width=True)
    with c2:
        ...
        st.plotly_chart(px.bar(pc, x='wickets', y='bowler', orientation='h', title="Purple Cap", color='wickets'), use_container_width=True)
...
if feature == "Head-to-Head":
    ...
    if ta != tb:
        ...
        st.plotly_chart(px.bar(h2h_df['winner'].value_counts().reset_index(), x='winner', y='count', color='winner'), use_container_width=True)
...
if feature == "Player Deep-Dive":
    ...
    st.metric(f"Total Runs: {player}", runs)
    ...
    st.plotly_chart(px.area(yearly, x='season', y='total_runs', title="Career Growth"), use_container_width=True)
...
if feature == "Venue & Toss Expert":
    ...
    st.plotly_chart(px.pie(v_data, names='toss_decision', title="Toss Decisions"), use_container_width=True)
    ...
    if t1 != t2:
        if st.button("Analyze Probabilities"):
            ...
            if h2h.shape[0] > 0:
                ...
                st.info("No direct history found.")
